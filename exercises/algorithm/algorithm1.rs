...
impl<T> LinkedList<T> {
        ...
        T: Ord + Copy,
    {
        ...
        while i < list_a.length as usize && j < list_b.length as usize {
            let va = *list_a.get(i as i32).unwrap();
            let vb = *list_b.get(j as i32).unwrap();
            if va <= vb {
                result.add(va);
                i += 1;
            } else {
        ...
        while i < list_a.length as usize {
            let va = *list_a.get(i as i32).unwrap();
            result.add(va);
            i += 1;
        }

        while j < list_b.length as usize {
            let vb = *list_b.get(j as i32).unwrap();
            result.add(vb);
            j += 1;
        }
